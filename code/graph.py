    ...
    for (name, typ) in it.all_action_types:
        for (elem_name, elem) in elems:
            ...
            for amt in range(0,3):
                ...
                color = 'r--'
                ys = [it.payoff_eval(actions = actions, p1_combo = combo, p2_disadvantage = it.Disadvantage(typ, elem, amt), p1_stance = it.Stance(combo.action1.type, amt2))[1][0] for amt2 in xs]
